...
const callApiSaga = function* () {
    yield effects.takeEvery("*", fetchData);
};
function* fetchData(request) {
    ...
    try {
        ...
        yield effects.put({ type: request.payload.actionTypes[1], payload });
    }
    catch (error) {
        yield effects.put({ type: request.payload.actionTypes[2], error });
    }
}
